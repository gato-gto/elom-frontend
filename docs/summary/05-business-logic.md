...
### Иерархия ролей (обновлено декабрь 2025)

> **Важно:** Введена новая система ролей с поддержкой операций "без учета изменений"

```mermaid
graph TD
    A[Admin] --> |Полный доступ| G[Все функции]
    M[Manager] --> |Без учета| H[Планирование]
    W[Warehouse] --> |Без учета| I[Складские операции]
    B[Brigadier] --> |Свои объекты| J[Закупки, списания]
    R[Requester] --> |Только заявки| K[Просмотр, заявки]
```
...
**Права доступа:**
- Полный доступ ко всем данным и функциям
- Создание объектов, материалов, закупок, списаний
- **БЕЗ УЧЕТА ИЗМЕНЕНИЙ**: Операции не создают записи в StockSnapshot

**Ограничения:**
- Операции не влияют на остатки материалов
- Не может изменять системные настройки
- Не может управлять пользователями

**Бизнес-функции:**
- Планирование закупок и списаний
- Документирование операций
- Стратегическое планирование
- Анализ эффективности

**Особенность "без учета изменений":**
- Закупки и списания создаются нормально
- НО: записи в StockSnapshot НЕ создаются
- Это позволяет планировать операции без изменения фактических остатков

#### 3. Brigadier (Бригадир/Инженер)
**Права доступа:**
- Создание объектов
- Создание и управление закупками (только для своих объектов)
- Создание списаний со склада (только для своих объектов)
- Автоматическое назначение ответственным за объект при первом списании

**Ограничения:**
- Только объекты, где бригадир ответственный или закреплен
- Не может изменять системные настройки
...
**Бизнес-функции:**
- Складские операции
- Документирование движений
- Планирование поставок
- Учет материалов на складе

**Особенность "без учета изменений":**
- Аналогично роли Manager
- Операции создаются, но не влияют на остатки

#### 5. Requester (Просмотр и подача заявки)
**Права доступа:**
- Просмотр материалов, объектов, закупок (только назначенные объекты)
- Создание заявок на материалы
- Просмотр своих заявок

**Ограничения:**
- Только назначенные объекты
- Не может создавать закупки и списания
- Не может редактировать справочники
- Только просмотр и создание заявок

**Бизнес-функции:**
- Просмотр доступных материалов
- Подача заявок на материалы
...
def get_accessible_objects(self):
    ...
    # Бригадир - объекты где он ответственный или закреплен
    if self.role == "brigadier":
        return Object.objects.filter(
            Q(responsible=self) | Q(assigned_employees=self)
        ).filter(is_active=True).distinct()
    
    # Requester - только назначенные объекты
    if self.role == "requester":
        return self.assigned_objects.filter(is_active=True)
    
    # По умолчанию - только назначенные объекты
    return self.assigned_objects.filter(is_active=True)
```

### Практическое применение
- **Admin/Manager/Warehouse**: Видят все объекты в системе
- **Brigadier**: Видят объекты где он ответственный или закреплен
- **Requester**: Видят только назначенные объекты

### Операции "без учета изменений"

Роли `manager` и `warehouse` работают в режиме "без учета изменений":

```python
def should_create_stock_snapshot(user) -> bool:
    """Проверяет, нужно ли создавать StockSnapshot для пользователя"""
    role = getattr(getattr(user, "profile", None), "role", None)
    # Роли без учета изменений
    no_accounting_roles = ["manager", "warehouse"]
    return role not in no_accounting_roles
```
...
- Документирование операций
- Анализ потребностей
- Без изменения фактических остатков
...
2. **Проверка прав доступа** - только admin, manager, warehouse могут закрывать периоды
...
1. **Проверка прав доступа** - только admin, manager могут открывать периоды
...
### Пользователи
1. **Телефон**: Обязателен для всех ролей кроме admin/manager
2. **Роли**: 5 ролей: admin, manager, brigadier, warehouse, requester
3. **Объекты**: Ограничение доступа по назначенным объектам
4. **Операции без учета**: manager и warehouse не создают StockSnapshot

### Заявки на материалы
1. **Создание**: Requester и Brigadier могут создавать заявки
2. **Одобрение**: Admin, Manager, Warehouse могут одобрять заявки
3. **Статусы**: pending, approved, rejected, fulfilled
4. **Связь**: Заявка привязана к объекту, материалу, количеству
