...
<template>
  <div class="list-container">
    ...
    <div v-if="tab === 'categories'">
      <div class="flex justify-end mb-2" v-if="canCreateCat">
        <button class="btn btn-sm btn-primary" @click="openCatForm(null)">
      ...
      <div class="overflow-x-auto">
        <table class="modern-table w-full">
          ...
          <tbody>
            <tr v-for="c in catStore.items" :key="c.id">
              <td class="font-medium">{{ c.name }}</td>
              <td>{{ c.parent_name || '— корневой (A) —' }}</td>
              <td class="text-right">{{ c.children_count }}</td>
              <td class="text-right">{{ c.items_count }}</td>
              <td class="text-right" v-if="canEditCat || canDeleteCat">
    ...
    <!-- Модалка раздела -->
    <Modal v-model="catModal" :title="editingCat?.id ? 'Изменить раздел' : 'Новый раздел'">
      <div class="p-4 space-y-3">
        <div class="form-control"><label class="label"><span class="label-text">Название</span><span class="label-text-alt text-error">*</span></label>
          <input v-model="catForm.name" type="text" maxlength="128" class="input input-bordered" :class="{ 'input-error': catErr.name }" />
          <label v-if="catErr.name" class="label"><span class="label-text-alt text-error">{{ catErr.name }}</span></label>
        </div>
        <div class="form-control"><label class="label"><span class="label-text">Родитель (A-раздел)</span></label>
          <select v-model.number="catForm.parent" class="select select-bordered" :class="{ 'select-error': catErr.parent }">
            <option :value="null">— корневой (A) —</option>
            <option v-for="c in roots" :key="c.id" :value="c.id" :disabled="c.id === editingCat?.id">{{ c.name }}</option>
          </select>
          <label v-if="catErr.parent" class="label"><span class="label-text-alt text-error">{{ catErr.parent }}</span></label>
        </div>
...
const catForm = reactive<{ name: string; parent: number | null }>({ name: '', parent: null })
const catErr = reactive<Record<string, string>>({})
const savingCat = ref(false)

function openCatForm(c: WorkCategory | null) {
  editingCat.value = c
  catForm.name = c?.name || ''
  catForm.parent = c?.parent ?? null
  catErr.name = ''; catErr.parent = ''
  catModal.value = true
}
async function saveCat() {
  ...
  try {
    if (editingCat.value?.id) {
      await catStore.update(editingCat.value.id, { name: catForm.name.trim(), parent: catForm.parent })
    } else {
      await catStore.create({ name: catForm.name.trim(), parent: catForm.parent })
    }
    ui.toast({ type: 'success', text: 'Раздел сохранён' })
    catModal.value = false
    await reloadCats()
  } catch (e: any) {
    const d = e?.response?.data
    if (d?.name) { catErr.name = Array.isArray(d.name) ? d.name[0] : String(d.name) }
    if (d?.parent) { catErr.parent = Array.isArray(d.parent) ? d.parent[0] : String(d.parent) }
    if (!catErr.name && !catErr.parent) { ui.toast({ type: 'error', text: d?.detail || 'Не удалось сохранить раздел' }) }
  } finally { savingCat.value = false }
}
async function deleteCat(c: WorkCategory) {
  if (!confirm(`Удалить раздел «${c.name}»?`)) { return }
  try {
    await catStore.remove(c.id)
    ui.toast({ type: 'success', text: 'Раздел удалён' })
    await reloadCats()
  } catch (e: any) {
    // BE delete-guard: нельзя удалить раздел с детьми/позициями → 400 с текстом.
    ui.toast({ type: 'error', text: e?.response?.data?.detail || 'Нельзя удалить раздел с подразделами или позициями' })
  }
}
