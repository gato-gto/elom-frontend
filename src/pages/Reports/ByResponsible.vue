  ...
  <div class="grid gap-4">
    <div class="card bg-white border">
      <div class="card-body grid md:grid-cols-5 gap-4">
        <fieldset class="fieldset">
          <label class="label" for="rr-from"><span class="label-text">Дата с</span></label>
          <input id="rr-from" v-model="dateFrom" type="date" class="input input-bordered input-sm"/>
        </fieldset>
        <fieldset class="fieldset">
          <label class="label" for="rr-to"><span class="label-text">Дата по</span></label>
          <input id="rr-to" v-model="dateTo" type="date" class="input input-bordered input-sm"/>
        </fieldset>
        <fieldset class="fieldset md:col-span-2">
          <label class="label" for="rr-search"><span class="label-text">Поиск</span></label>
          <input id="rr-search" v-model.trim="search" class="input input-bordered input-sm" placeholder="объект/поставщик/комментарий" @keyup.enter="load"/>
        </fieldset>
    ...
    <div class="overflow-auto border border-gray-200 rounded-xl">
      <table class="table table-zebra w-full">
        ...
        <tbody>
        <tr v-for="r in rows" :key="r.responsible">
...
type Row = { responsible: number; responsible_name: string; total_amount: string; purchases_count?: number }
...
async function load() {
  ...
  try {
    ...
    const {data} = await api.get<ReportResponse>(endpoints.reports.byResponsible + q)
