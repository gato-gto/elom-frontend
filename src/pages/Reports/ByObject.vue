    ...
    <div class="overflow-auto border border-base-300 rounded-xl">
      <table class="table table-zebra w-full">
        ...
        <tr v-for="(r, i) in rows" :key="i">
          <td>{{ r.date ?? '—' }}</td>
          <td>{{ r.object ?? objectName(r.object_id) ?? '—' }}</td>
          <td>{{ r.responsible ?? '—' }}</td>
          <td>{{ r.material ?? materialName(r.material_id) ?? '—' }}</td>
          <td>{{ r.unit_name ?? '—' }}</td>
...
import type {PageResponse, ReportRow, ReportResponse, ReportCommonQuery, SiteObject, EmployeeListItem, Material} from '@/api/types'
...
const filters = ref<ReportCommonQuery>({from: undefined, to: undefined, object: undefined, responsible: undefined})
...
async function fetchReport() {
  loading.value = true
  try {
    const q = {...filters.value, page: page.value, page_size: pageSize} as ReportCommonQuery & { page: number; page_size: number }
    const {data} = await api.get<ReportResponse>(endpoints.reports.byObject + buildQuery(q as Query))
    if (Array.isArray(data)) {
      rows.value = data
      count.value = data.length
    } else {
      rows.value = (data as PageResponse<ReportRow>).results
      count.value = (data as PageResponse<ReportRow>).count
    }
  } finally {
    loading.value = false
  }
...
const exportUrl = computed(() => {
  const q = {...filters.value, export: 'xlsx'} as ReportCommonQuery
