...
async function loadObjects() {
    const { data } = await api.get(endpoints.objects.list + buildQuery({ page_size: 1000 }));
...
async function load() {
    ...
    try {
        ...
        const { data } = await api.get(endpoints.reports.byMaterial + q);
...
__VLS_asFunctionalElement(__VLS_elements.input)({
    ...
    id: "rm-search",
    ...{ class: "input input-bordered input-sm" },
    placeholder: "название материала / SKU",
});
(__VLS_ctx.search);
// @ts-ignore
[load, search,];
__VLS_asFunctionalElement(__VLS_elements.div, __VLS_elements.div)({
    ...{ class: "flex items-end gap-2 md:col-span-2" },
});
