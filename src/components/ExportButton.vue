<template>
  ...
  <details ref="dd" class="dropdown dropdown-end w-full md:w-auto">
    ...
    <ul class="dropdown-content menu p-2 shadow rounded-box w-52 z-50">
      ...
      <!-- F-866: PDF убран — серверный экспорт списков в PDF не реализован (возвращал JSON →
           ошибка «формат не поддерживается»). Остаются CSV и Excel (оба через бэкенд, полные данные). -->
    </ul>
  </details>
</template>
...
function pick(format: 'csv' | 'excel') {
