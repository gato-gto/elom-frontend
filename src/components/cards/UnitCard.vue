  ...
  >
    <template #content>
      <div class="space-y-2 text-sm">
        <div>
          <span class="text-gray-500">Код единицы:</span>
        ...
        <div v-if="unit.materials_count">
          <span class="text-gray-500">Материалов:</span>
          <span class="font-medium ml-2">{{ unit.materials_count }}</span>
        </div>
        
        <div v-if="unit.conversions_count">
          <span class="text-gray-500">Конверсий:</span>
          <span class="font-medium ml-2">{{ unit.conversions_count }}</span>
        </div>
        
        <div v-if="unit.purchases_count">
          <span class="text-gray-500">В закупках:</span>
          <span class="font-medium ml-2">{{ unit.purchases_count }}</span>
        </div>
        
        <div v-if="unit.writeoffs_count">
          <span class="text-gray-500">В списаниях:</span>
          <span class="font-medium ml-2">{{ unit.writeoffs_count }}</span>
        </div>
        
        <div v-if="unit.conversions && unit.conversions.length > 0">
          <span class="text-gray-500">Конверсии:</span>
          <div class="font-medium ml-2">
            <div class="space-y-1 mt-1">
              ...
              <div v-if="unit.conversions.length > 3" class="text-xs text-gray-500">
        ...
        <div v-if="unit.example_materials && unit.example_materials.length > 0">
          <span class="text-gray-500">Используется в материалах:</span>
          <div class="font-medium ml-2">
    ...
    <template #extra>
      <div class="flex justify-between items-center text-xs text-gray-500">
        <span>Создана: {{ formatDate(unit.created_at || null) }}</span>
        <span v-if="unit.updated_at !== unit.created_at">
