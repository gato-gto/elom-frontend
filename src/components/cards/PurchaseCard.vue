<template>
  ...
  >
    <template #content>
      <div class="space-y-2 text-sm">
        ...
        <div v-if="purchase.supplier">
          <span class="text-base-content/60">Поставщик:</span>
          <span class="font-medium ml-2">{{ purchase.supplier }}</span>
        </div>
        
        <div>
          <span class="text-base-content/60">Сумма:</span>
        ...
        <div v-if="purchase.status === 'completed' && purchase.has_report_photos === false">
          <span class="text-base-content/60">Фото-отчёт:</span>
    ...
    <template #extra>
      <div class="flex justify-between items-center text-xs text-base-content/60 font-mono">
